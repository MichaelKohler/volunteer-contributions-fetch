...
export function validate(config) {
  debug('Validating OSM config');

  if (!config) {
    return;
  }

  if (config && config.enabled && !config.displayName) {
    throw new Error('OSM: displayName is required');
  }

  if (config && config.enabled && !config.stopDate) {
    throw new Error('OSM: stopDate is required');
  }
}
