...
function validate(config) {
  ...
  if (config && config.enabled && !config.participationType) {
    throw new Error('Community Portal: participationType is required');
  }

  if (config && config.enabled && !config.campaignType) {
    throw new Error('Community Portal: campaignType is required');
  }
}
...
async function processEvents(communityPortal, document) {
  ...
  const allEvents = Array.from(
    allEventNodes,
    (entity) => formatEvent(communityPortal.participationType, entity)
  )
...
async function processCampaigns(communityPortal, document) {
  ...
  return Array.from(
    allCampaignNodes,
    (entity) => formatCampaign(communityPortal.campaignType, entity)
  )
    .sort((a, b) => b.createdAt - a.createdAt)
