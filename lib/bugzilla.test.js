...
test.beforeEach((t) => {
  ...
  axios.request.onCall(0).resolves({ data: firstBugsPage });
  axios.request.onCall(1).resolves({ data: secondBugsPage });
});
...
test.serial('should ignore comments older than stopDate', async (t) => {
  const bugsPage = {
    bugs: [
      {
        created_at: new Date('2022-04-13'),
  ...
  axios.request.onCall(0).resolves({ data: bugsPage });
  axios.request.onCall(1).resolves({ data: oldComments });
