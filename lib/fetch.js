...
async function fetchAll(config) {
  ...
  debug('fetching...');
  const existingContributions = JSON.parse(await fs.readFile(config.outputFile, 'utf-8'));
  ...
  const discourseResult = await discourse.gather(config);
  ...
  await fs.writeFile(config.outputFile, `${JSON.stringify(uniqueContributions, null, 2)}\n`);
  debug('FILE_WRITTEN');
}
