...
import { gather, validate } from './community-portal.js';
...
test.serial(
  'should only fetch events and campaigns that are not in the future',
  async (t) => {
...
test.serial('should format with custom types', async (t) => {
  ...
  // Event
  t.is(contributions[0].type, specificTypeConfig.participationType);
  t.is(contributions[1].type, specificTypeConfig.participationType);
  t.is(contributions[2].type, specificTypeConfig.campaignType);
  t.is(contributions[3].type, specificTypeConfig.campaignType);
});
