...
function validate(config) {
  ...
  if (config && config.enabled && !config.topicType) {
    throw new Error('Discourse: topicType is required');
  }

  if (config && config.enabled && !config.postType) {
    throw new Error('Discourse: postType is required');
  }
}

async function gather({ discourse }) {
...
async function processTopics({ baseUrl, username, topicType }) {
  ...
  for await (const topicsResponse of page) {
    ...
    allTopics = allTopics.concat(topics.map((entity) => formatTopic(topicType, baseUrl, entity)));
...
async function processPosts({ baseUrl, username, postType }) {
  ...
  for await (const postsResponse of page) {
    ...
    allPosts = allPosts.concat(posts.map((entity) => formatPost(postType, baseUrl, entity)));
