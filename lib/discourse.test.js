...
import { gather, validate } from './discourse.js';
...
beforeEach(() => {
  vi.spyOn(axios, 'get')
    ...
    .mockResolvedValueOnce({ data: TOPICS_SMALL })
    .mockResolvedValueOnce({ data: POSTS })
    .mockResolvedValueOnce({ data: POSTS_SMALL });
});
...
test('should keep deleted post', async () => {
  ...
  vi.spyOn(axios, 'get')
    .mockResolvedValueOnce({ data: TOPICS_EMPTY })
    .mockResolvedValueOnce({ data: POSTS_SMALL });
