...
async function gather({ github }, existingContributions = []) {
  ...
  const latestIssue = existingContributions.find((contribution) => {
    return (
      contribution.source === GITHUB_ISSUES_CREATED_CATEGORY ||
  ...
  try {
    ...
    if (error.message === 'PRIVATE_SCOPE') {
      if (!github.allowPrivate) {
        throw new Error(
          'GITHUB_TOKEN has private repo scope, but the config does not specify allowPrivate to fetch private repos. Are you sure you want to fetch private information?'
        );
      }
    } else {
      throw error;
  ...
  const contributions = [...commits, ...issues, ...reviews, ...comments];
