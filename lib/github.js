...
async function processIssues(github, latestCreationDate) {
  if (typeof github.issuesEnabled !== 'undefined' && github.issuesEnabled === false) {
  ...
  for await (const periodChunk of generatedPeriod) {
    const currentPage = pageGenerator(
      github,
      periodChunk,
      (period, page) => getIssuePage(github, 'author', period, page)
    );
...
async function processCommits(github, latestCreationDate) {
  if (typeof github.commitsEnabled !== 'undefined' && github.commitsEnabled === false) {
  ...
  for await (const periodChunk of generatedPeriod) {
    const currentPage = pageGenerator(
      github,
      periodChunk,
      (period, page) => getCommitPage(github, period, page)
    );
...
async function processReviews(github, latestCreationDate) {
  if (typeof github.reviewsEnabled !== 'undefined' && github.reviewsEnabled === false) {
  ...
  const prReviewsList = getPRReviews(allPRs.flat(), github.username, github.delayMsPerRequest);
  for await (const prReviews of prReviewsList) {
    allReviews.push(prReviews);
  }

  const processedReviews = processEntities(github, allReviews.flat(), prTitleMap);
...
async function processComments(github, latestCreationDate) {
  if (typeof github.commentsEnabled !== 'undefined' && github.commentsEnabled === false) {
  ...
  allCommentedIssues.flat()
    .forEach((commentIssue) => commentTitleMap.set(commentIssue.url, commentIssue.title));
  ...
  const processedComments = processEntities(github, allComments.flat(), commentTitleMap);
...
async function getCommitPage({ username, delayMsPerRequest }, commitsPeriod, page) {
...
async function getIssuePage(github, relationship, issuesPeriod, page) {
  ...
  try {
    const additionalFields = relationship === 'commenter' ? '+is:issue' : '';
    const items = await fetcher.searchIssues({
      q: `${relationship}:${github.username}+created:${after}..${before}${additionalFields}`,
      sort: 'created',
      page,
    });
...
async function* getPRReviews(prList, reviewerUsername, delay) {
  ...
  for await (const pr of prList) {
    try {
      ...
      const onlyMyReviews = items.filter((review) => review.user.login === reviewerUsername);
...
async function* getComments(issueList, commenterUsername, delay) {
  ...
  for await (const issue of issueList) {
    try {
      const [, owner, repo] = /github\.com\/repos\/(.+)\/(.*)/.exec(issue.repository_url);
      // We're waiting for a bit here, as we've previously hit a secondary GitHub rate limit...
      await waitFor(delay ?? DELAY_FETCHING_MS);
      debug(`Fetching comments for ${owner}/${repo} issue ${issue.number}`);
      const items = await fetcher.listComments({
        owner,
        repo,
        issue_number: issue.number,
      });

      const onlyMyComments = items.filter((comment) => comment.user.login === commenterUsername);
