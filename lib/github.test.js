...
test.serial('should validate config - missing completely', (t) => {
  const emptyConfig = {};
  t.notThrows(() => github.validate(emptyConfig));
});
...
test.serial('should validate config - missing filter', (t) => {
  ...
  t.throws(() => github.validate(failConfig));
});
...
//   const contributions = await github.gather(config);
...
test.serial('should not fetch if disabled - by enable property', async (t) => {
  ...
  const contributions = await github.gather(specificTypeConfig);

  t.is(contributions.length, 0);
});

test.serial('should not fetch if disabled - by leaving off config value', async (t) => {
  const contributions = await github.gather({});

  t.is(contributions.length, 0);
});

test.serial('should throw if private repo scope token is used and config does not allow private repo access', async (t) => {
  fetcher.throwIfTokenHasPrivateRepoScope.rejects(new Error('PRIVATE_SCOPE'));

  await t.throwsAsync(() => github.gather(config));
});

test.serial('should not throw if private repo scope token is used but config does allow private repo access', async (t) => {
  fetcher.throwIfTokenHasPrivateRepoScope.rejects(new Error('PRIVATE_SCOPE'));
  ...
  t.notThrows(() => github.gather(allowedPrivateConfig));
});

test.serial('should throw if error is not specifically private scope error', async (t) => {
  fetcher.throwIfTokenHasPrivateRepoScope.rejects(new Error('SOMETHING_ELSE'));
...
test.serial('should fetch commits until stop date', async (t) => {
  ...
  fetcher.searchCommits.onCall(0).resolves(new Array(100).fill(0).map(() => ({ createdAt: new Date('2022-03-01') })));
  fetcher.searchCommits.onCall(1).resolves(new Array(5).fill(0).map(() => ({ createdAt: new Date('2022-02-01') })));
  fetcher.searchCommits.onCall(2).resolves(new Array(5).fill(0).map(() => ({ createdAt: new Date('2022-01-01') })));

  await github.gather(commitConfig);
...
test.serial('should format commits', async (t) => {
  ...
  const result = await github.gather(commitConfig);
...
test.serial('should fetch issues until stop date', async (t) => {
  ...
  fetcher.searchIssues.onCall(0).resolves(new Array(100).fill(0).map(() => ({ createdAt: new Date('2022-03-01') })));
  fetcher.searchIssues.onCall(1).resolves(new Array(5).fill(0).map(() => ({ createdAt: new Date('2022-02-01') })));
  fetcher.searchIssues.onCall(2).resolves(new Array(5).fill(0).map(() => ({ createdAt: new Date('2022-01-01') })));

  await github.gather(commitConfig);
  t.deepEqual(fetcher.searchIssues.getCall(0).args[0], {
    q: `author:${config.github.username}+created:2022-03-01..2022-03-31`,
    sort: 'created',
    page: 1,
  });
  ...
  t.deepEqual(fetcher.searchIssues.getCall(2).args[0], {
    q: `author:${config.github.username}+created:2022-02-01..2022-02-28`,
    sort: 'created',
    page: 1,
  });
  t.deepEqual(fetcher.searchIssues.getCall(3).args[0], {
    q: `author:${config.github.username}+created:2022-01-01..2022-01-31`,
    sort: 'created',
    page: 1,
  });
});
...
test.serial('should fetch reviews until stop date', async (t) => {
  ...
  // We expect 3 periods to be queried based on the passed stopDate and the faked clock.
  fetcher.searchIssues.resolves([]);
  fetcher.searchIssues.onCall(0).resolves(new Array(100).fill(0).map(() => ({
  ...
  fetcher.searchIssues.onCall(1).resolves(new Array(5).fill(0).map(() => ({
  ...
  fetcher.searchIssues.onCall(2).resolves(new Array(5).fill(0).map(() => ({
  ...
  fetcher.listReviews.onCall(0).resolves([{ createdAt: new Date('2022-03-01') }]);

  await github.gather(commitConfig);
  t.deepEqual(fetcher.searchIssues.getCall(0).args[0], {
    q: `reviewed-by:${config.github.username}+created:2022-03-01..2022-03-31`,
    sort: 'created',
    page: 1,
  });
  t.deepEqual(fetcher.searchIssues.getCall(1).args[0], {
  ...
  t.deepEqual(fetcher.searchIssues.getCall(2).args[0], {
    q: `reviewed-by:${config.github.username}+created:2022-02-01..2022-02-28`,
    sort: 'created',
    page: 1,
  });
  t.deepEqual(fetcher.searchIssues.getCall(3).args[0], {
    q: `reviewed-by:${config.github.username}+created:2022-01-01..2022-01-31`,
    sort: 'created',
    page: 1,
  });
...
test.serial('should format comments', async (t) => {
  ...
  fetcher.listComments.onCall(0).resolves([{
    created_at: new Date('2022-03-02'),
    issue_url: 'https://api.github.com/repos/contribution-test/foo/issues/1111',
    html_url: 'https://github.com/contribution-test/foo/issues/1111',
    user: {
      login: config.github.username,
    },
...
test.serial('should ignore comments not matching filter', async (t) => {
  ...
  fetcher.listComments.onCall(0).resolves([{
    created_at: new Date('2022-03-02'),
    issue_url: 'https://api.github.com/repos/not-matching-filter/foo/issues/1111',
    html_url: 'https://github.com/not-matching-filter/foo/issues/1111',
    user: {
      login: config.github.username,
    },
