...
function validate(config) {
  ...
  if (config && config.enabled && !config.baseUrl) {
    throw new Error('Wiki: baseUrl is required');
  }

  if (config && config.enabled && !config.username) {
    throw new Error('Wiki: username is required');
  }

  if (config && config.enabled && !config.editType) {
    throw new Error('Wiki: editType is required');
  }

  if (config && config.enabled && !config.stopDate) {
    throw new Error('Wiki: stopDate is required');
  }
}

async function gather({ wiki }) {
  const edits = await processEdits(wiki);
...
async function processEdits({
  ...
  for await (const periodChunk of period) {
    ...
    const processed = processEntities(type, edits, periodChunk);
...
function processEntities(type, entities, periodChunk) {
  return entities.map((entity) => format(type, entity)).filter((entity) => {
    return entity.createdAt.getMonth() + 1 === periodChunk[0]
